...
from src.pykiwoom.kiwoom_proxy import KiwoomProxy
...
class KiwoomManager:
    ...
    # 내가 추가한 함수
    def get_chejan_no_blocking(self):
        if self.chejan_dqueue.empty():
            return None
        return self.chejan_dqueue.get(block=False)

    # 내가 추가한 함수
    def get_real_no_blocking(self):
        if self.real_dqueues.empty():
            return None
        return self.real_dqueues.get()
